...
let raToHa y m d t lon ra (ha: float) =
...
let haToRa y m d t lon ha (ra: float) =
...
let meanObliquity (y: int) m d (ob: float) =
...
let eclToEq lon lat (year: int) month day ra dec =
...
let eqToEcl ra dec (year: int) month day lon lat =
...
let celestialAngle x1 y1 x2 y2 (d: float) =
...
let ``risingAndSetting some value returned`` (y: int) m d v lat lon ra dec azR utR azS utS =
    let result = 
        risingAndSetting 
            (new DateTime(y, m, d)) 
            v 
...
let ``risingAndSetting none value returned`` (y: int) m d v lat lon ra dec =
    let result = 
        risingAndSetting 
            (new DateTime(y, m, d)) 
            v 
...
let precessionLowPrecision epoch (y: int) m d ra dec raP decP =
...
let nutation (y: int) m d lon obl =
...
[<TestCase(60.0, 1.02, 1979, 2, 26, 10, 45, 0, 50.0, -100.0, 338.82916666666665, -7.686944444444444, 339.18008100000003, -8.5381655)>]
